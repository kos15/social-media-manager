...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const post of duePosts) {
      console.log(
        `[Cron Publisher] Processing post ${post.id} for user ${post.userId}`,
      );
      ...
      if (userAccounts.length === 0) {
        successAll = false;
        errors.push(
          "No active connected accounts found for the configured platforms.",
        );
      }
    ...
    return NextResponse.json(
      { error: errorMessage || "Internal cron error" },
      { status: 500 },
    );
  }
}
