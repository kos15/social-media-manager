...
import {
    ...
    Eye, EyeOff, ExternalLink, Loader2, Bell, User
} from "lucide-react";
...
const PLATFORM_CONFIG: Record<Platform, {
    ...
    devUrl: string;
    callbackPath: string;
    fields: { key: string; label: string; placeholder: string; required: true }[];
    notes: string;
}> = {
    TWITTER: {
        name: "Twitter / X",
        ...
        fields: [
            { key: "clientId", label: "Client ID (OAuth 2.0)", placeholder: "e.g. M1M5R3BMVy13QmpScXkzTUt5OE4...", required: true },
            { key: "clientSecret", label: "Client Secret", placeholder: "Enter your X app Client Secret", required: true },
        ],
        notes: "OAuth 2.0 with PKCE. In the Developer Portal → Your App → Settings → User authentication settings, enable Read + Write permissions, set App Type to 'Web App', and add the Callback URL above. Required scopes: tweet.read, tweet.write, users.read, offline.access.",
    },
    LINKEDIN: {
        name: "LinkedIn",
        icon: Linkedin,
        ...
        fields: [
            { key: "clientId", label: "Client ID", placeholder: "Enter your LinkedIn Client ID", required: true },
            { key: "clientSecret", label: "Client Secret", placeholder: "Enter your LinkedIn Client Secret", required: true },
        ],
        notes: "Requires 'openid', 'profile', 'email', and 'w_member_social' scopes on your LinkedIn app.",
    },
    INSTAGRAM: {
        name: "Instagram",
        icon: Instagram,
        ...
        fields: [
            { key: "clientId", label: "App ID (Client ID)", placeholder: "Enter your Meta App ID", required: true },
            { key: "clientSecret", label: "App Secret", placeholder: "Enter your Meta App Secret", required: true },
        ],
        notes: "Uses Instagram Basic Display API (Meta Developer App). Enable 'user_profile' and 'user_media' scopes.",
    },
    YOUTUBE: {
        name: "YouTube",
        icon: Youtube,
        ...
        fields: [
            { key: "clientId", label: "Google Client ID", placeholder: "Enter your Google OAuth Client ID", required: true },
            { key: "clientSecret", label: "Google Client Secret", placeholder: "Enter your Google Client Secret", required: true },
        ],
        notes: "Google OAuth 2.0 with YouTube Data API v3. Enable YouTube Data API v3 in your Google Cloud project.",
    },
};
...
function Toast({ message, type, onDismiss }: { message: string; type: "success" | "error"; onDismiss: () => void }) {
    return (
        <div onClick={onDismiss} className={`fixed top-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium cursor-pointer transition-all ${type === "success" ? "bg-success/10 text-success border border-success/20" : "bg-error/10 text-error border border-error/20"}`}>
...
function SettingsContent() {
    ...
    const [saved, setSaved] = useState<Record<Platform, SavedCredential | null>>({
        TWITTER: null, LINKEDIN: null, INSTAGRAM: null, YOUTUBE: null,
    });
    ...
    const handleSave = async (platform: Platform) => {
        ...
        try {
            const res = await fetch("/api/settings/credentials", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ platform, clientId: form.clientId, clientSecret: form.clientSecret }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error ?? "Failed to save");

            showToast(`${PLATFORM_CONFIG[platform].name} credentials saved!`, "success");
    ...
    const handleRemove = async (platform: Platform) => {
        ...
        try {
            const res = await fetch(`/api/settings/credentials?platform=${platform}`, { method: "DELETE" });
            if (!res.ok) throw new Error("Failed to remove");
            showToast(`${PLATFORM_CONFIG[platform].name} credentials removed.`, "success");
            setSaved(prev => ({ ...prev, [platform]: null }));
            updateForm(platform, { removing: false });
        } catch {
            updateForm(platform, { removing: false, error: "Failed to remove credentials" });
        }
    };
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            ...
            <div>
                ...
                <p className="text-sm text-text-secondary mt-1">Manage your account preferences and API integrations.</p>
            </div>
            ...
            <div className="flex gap-1 border-b border-border">
                {([
                    ...
                    { id: "integrations", label: "API Integrations", icon: ExternalLink },
                ] as const).map((tab) => (
                    <button
                        key={tab.id}
                ...
                <div className="bg-surface-elevated border border-border rounded-xl overflow-hidden">
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="pt-4 border-t border-border">
                            ...
                            <div className="space-y-4">
                                {[
                                    { label: "Publishing Success", desc: "Get notified when a post publishes successfully" },
                                    { label: "Publishing Failures", desc: "Get notified immediately if a post fails" },
            ...
            {activeTab === "integrations" && (
                <div className="space-y-4">
                    <div className="bg-surface-elevated border border-amber-400/30 rounded-xl p-4 flex gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-amber-400">API Credentials Required</p>
                            <p className="text-xs text-text-secondary mt-1">
                                Enter your OAuth app credentials for each platform. Both fields marked <span className="text-error font-bold">*</span> are required before you can connect an account.
                    ...
                    {loading ? (
                        ...
                        PLATFORMS.map(platform => {
                            ...
                            return (
                                <div key={platform} className="bg-surface-elevated border border-border rounded-xl overflow-hidden">
                                    {/* Header */}
                                    ...
                                    <div className="p-5 space-y-4">
                                            ...
                                            <div className="flex items-start justify-between bg-success/5 border border-success/20 rounded-lg px-4 py-3">
                                                <div className="text-xs space-y-1">
                                                    ...
                                                    <p className="text-text-secondary pt-1">
                                                        Updated {new Date(savedCred.updatedAt).toLocaleDateString()}
                                                    </p>
                                                </div>
                                                <button
                                        ...
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            ...
                                            <div className="grid gap-1.5">
                                                <label className="text-sm font-medium flex items-center gap-1">
                                                    Client ID
                                                    <span className="text-error font-bold" title="Required">*</span>
                                                </label>
                                                <input
                                                    type="text"
                                                    value={form.clientId}
                                                    onChange={e => updateForm(platform, { clientId: e.target.value, error: null })}
                                                    placeholder={config.fields[0].placeholder}
                                                    className={`w-full bg-surface border rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 transition-colors ${form.clientId.trim() === "" && !form.saving
                                            ...
                                            <div className="grid gap-1.5">
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        onChange={e => updateForm(platform, { clientSecret: e.target.value, error: null })}
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => updateForm(platform, { showSecret: !form.showSecret })}
                                        ...
                                        {(!form.clientId.trim() || !form.clientSecret.trim()) && form.clientId.trim() === "" && form.clientSecret.trim() === "" && (
                                            <p className="text-xs text-text-secondary flex items-center gap-1.5">
                                                <span className="text-error font-bold">*</span>
                                                Both Client ID and Client Secret are required to enable {config.name} integration.
                                            </p>
                                        )}
                                        ...
                                        <div className="bg-surface rounded-lg border border-border px-3 py-2 text-xs">
                                            <span className="text-text-secondary font-medium">Redirect / Callback URL: </span>
                                            <span className="font-mono text-primary">{origin}{config.callbackPath}</span>
                                        ...
                                        <div className="flex justify-end">
                                            <button
                                                ...
                                                className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium transition-all ${valid && !form.saving
                                                    ? "bg-primary hover:bg-primary-hover text-white cursor-pointer"
                                                    : "bg-surface text-text-secondary border border-border cursor-not-allowed opacity-60"
                                                    }`}
                                                title={!valid ? "Fill in both Client ID and Client Secret to save" : ""}
                                            >
                                                {form.saving
                                                    ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</>
                                                    : <><Save className="w-4 h-4" /> {savedCred ? "Update Credentials" : "Save Credentials"}</>
                                                }
                                            </button>
                                        </div>
                                    </div>
...
export default function SettingsPage() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center h-64 text-text-secondary">Loading settings...</div>}>
            <SettingsContent />
        </Suspense>
    );
}
