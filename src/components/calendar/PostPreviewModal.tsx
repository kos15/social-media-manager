      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white text-[15px] leading-relaxed mt-1 whitespace-pre-wrap break-words">
            {content || (
              <span className="text-slate-600">What&apos;s happening?</span>
            )}
          </p>
          {mediaUrls.length > 0 && (
    ...
    <div className="bg-[#1B1F23] rounded-xl border border-slate-700 overflow-hidden">
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          <button className="ml-auto text-[#0A66C2] border border-[#0A66C2] text-xs font-semibold px-3 py-1 rounded-full">
            + Follow
          </button>
        </div>
      ...
      <div className="px-4 py-3 border-t border-slate-700">
        ...
        <div className="flex justify-around">
          {[
            ...
            <button
              key={label}
              className="flex items-center gap-1.5 text-slate-400 text-xs font-medium px-2 py-1 rounded hover:bg-slate-700 transition"
            >
...
function InstagramCard({
  ...
  return (
    <div className="bg-black rounded-xl border border-slate-800 overflow-hidden">
      ...
      <div
        className={`w-full aspect-video flex items-center justify-center ${mediaUrls.length > 0 ? "" : "bg-gradient-to-br from-slate-800 to-slate-900"}`}
      >
        {mediaUrls.length > 0 ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
      ...
      <div className="p-3 space-y-2">
        ...
        <p className="text-white text-xs">
          ...
          <span className="text-slate-300">
            {content || (
              <span className="text-slate-600">Caption goes here...</span>
            )}
          </span>
        </p>
      </div>
...
export function PostPreviewModal({
  ...
  const { editPost, removeScheduledPost } = usePostStore();
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(30,32,44,0.98) 0%, rgba(20,22,35,0.98) 100%)",
        }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center ${published ? "bg-slate-500/20 border border-slate-500/30" : "bg-violet-500/20 border border-violet-500/30"}`}
            >
              {published ? (
        ...
        <div
          className={`overflow-y-auto flex-1 px-5 py-4 space-y-4 custom-scrollbar ${published ? "opacity-75" : ""}`}
        >
          {post.platforms.includes("TWITTER") && (
            <TwitterCard content={post.content} mediaUrls={post.mediaUrls} />
          )}
          {post.platforms.includes("LINKEDIN") && (
            <LinkedInCard content={post.content} mediaUrls={post.mediaUrls} />
          )}
          {post.platforms.includes("INSTAGRAM") && (
            <InstagramCard content={post.content} mediaUrls={post.mediaUrls} />
          )}
          {post.platforms.includes("YOUTUBE") && (
            <YouTubeCard content={post.content} />
          )}
          {post.platforms.length === 0 && (
            <div className="rounded-xl bg-white/5 border border-white/10 p-4">
              <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap break-words">
                {post.content || (
                  <span className="text-slate-600">No content</span>
                )}
              </p>
              {post.mediaUrls.length > 0 && (
                ...
                <img
                  ...
                  alt="Media"
                  className="mt-3 rounded-lg w-full object-cover max-h-[200px]"
                />
              )}
            </div>
          )}
        ...
        <div className="px-5 py-4 border-t border-white/10 flex items-center justify-end gap-2 shrink-0">
          ...
          {/* Delete and Edit only for future (not yet published) posts */}
          {!published && (
            <>
              <button
                onClick={handleDelete}
                disabled={deleting}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-red-600/20 hover:bg-red-600/30 text-red-400 border border-red-600/30 transition-colors disabled:opacity-50"
              >
                {deleting ? (
